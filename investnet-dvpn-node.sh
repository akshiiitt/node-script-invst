...
WG_SERVICE_CONFIG="${NODE_DIR}/wireguard/config.toml"
DENOM="invst"
...
# Sync wireguard settings from wg0.conf into node's wireguard sub-config
update_wireguard_config_from_wg() {
  ...
  if sudo test -f "$WG_CONF"; then
    ...
    # echo "- PrivateKey: $PRIVATE_KEY" # Masked for security
  else
    echo "WireGuard config not found at $WG_CONF"
  fi

  # Update ~/.investnet-dvpnx/wireguard/config.toml (InvestNet specific path)
  if [[ -f "$WG_SERVICE_CONFIG" ]]; then
    # Only update when all required fields are available
    if [[ -n "${WG_ADDRESS:-}" && -n "${WG_LISTENPORT:-}" && -n "${PRIVATE_KEY:-}" ]]; then
      echo "Updating InvestNet WireGuard service config at $WG_SERVICE_CONFIG"
      # Create a backup before in-place edits
      cp "$WG_SERVICE_CONFIG" "${WG_SERVICE_CONFIG}.bak.$(date +%s)"
      
      # InvestNet uses a flat key structure in wireguard/config.toml
      sed -i -E "s|^ipv4_addr = .*|ipv4_addr = '${WG_ADDRESS}'|" "$WG_SERVICE_CONFIG"
      sed -i -E "s|^port = .*|port = '${WG_LISTENPORT}'|" "$WG_SERVICE_CONFIG"
      sed -i -E "s|^private_key = .*|private_key = '${PRIVATE_KEY}'|" "$WG_SERVICE_CONFIG"
    else
      echo "WireGuard values missing from $WG_CONF; skipping $WG_SERVICE_CONFIG update"
    fi
  else
    echo "WireGuard service config not found at $WG_SERVICE_CONFIG"
    # If the main node is initialized but the wireguard subfolder isn't, something is wrong
    # though it might be created on first start if not existing.
  fi
}
...
if ! command -v go &> /dev/null; then
  echo "Go not found.  Installing Go with ./install-go.sh is recommended."
  echo "After go install run 'source ~/.profile' to update your environment."
  # exit 1 # Let's not hard exit if we're just running binary, but helpful check
else
...
if ! command -v "${BINARY}" &> /dev/null; then
  ...
  # Optional: Try to get existing WG settings from /etc/wireguard to use during init
  WG_INIT_PORT="51820"
  WG_INIT_KEY=""
  if sudo test -f "$WG_CONF"; then
...
function cmd_start {
    ...
    # Sync [wireguard] sub-config in wireguard/config.toml from /etc/wireguard/wg0.conf
    update_wireguard_config_from_wg
  ...
  echo "Starting node with command:"
    ...
    if [[ -n "$PUB_IP" ]]; then
      # Try with curl to see if API is up
      NODE_ADDR=$(curl -sk "https://$PUB_IP:$API_PORT_CFG" | jq -r '.result.addr' 2>/dev/null || true)
      if [[ -n "$NODE_ADDR" && "$NODE_ADDR" != "null" ]]; then
        log "Node address: $NODE_ADDR"
      else
        log "Node API reachable but address not yet available or jq missing"
      fi
    fi
}
